<x-guest-layout title="Log In">
    ...
    <p class="mt-1.5 text-sm text-muted">Masuk to manage your bookings and trips.</p>

    <x-auth-session-status class="mt-4" :status="session('status')" />

    <form method="POST" action="{{ route('login') }}" class="mt-6 space-y-4" x-data="{ showKata Sandi: false }">
        @csrf
        ...
        <div>
            ...
            <div class="relative mt-1.5">
                <input :type="showKata Sandi ? 'text' : 'password'" id="password" name="password" required autocomplete="current-password" class="input pr-10">
