<x-app-layout title="Booking Detail">
    ...
    <div class="card p-6">
        ...
        <div class="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm border-t border-slate-100 pt-5">
            ...
            <div><p class="text-muted">Departure</p><p class="font-medium text-ink">{{ optional($booking->availability)->departure_date?->format('d M Y') }}</p></div>
            <div><p class="text-muted">Return</p><p class="font-medium text-ink">{{ optional($booking->availability)->return_date?->format('d M Y') }}</p></div>
            <div><p class="text-muted">Booked On</p><p class="font-medium text-ink">{{ $booking->created_at->format('d M Y') }}</p></div>
        </div>

        <div class="mt-5 border-t border-slate-100 pt-5">
            <h3 class="font-semibold text-ink mb-2">Participants</h3>
            <div class="flex gap-6 text-sm text-muted">
                <span>Adults: <span class="text-ink font-medium">{{ $booking->adult_count }}</span></span>
                <span>Children: <span class="text-ink font-medium">{{ $booking->child_count }}</span></span>
                <span>Infants: <span class="text-ink font-medium">{{ $booking->infant_count }}</span></span>
            </div>
        </div>
        ...
        @if (! in_array($booking->status, ['cancelled', 'completed']))
            <div class="mt-6 border-t border-slate-100 pt-5 flex flex-wrap gap-3">
                @if ((float) $booking->balance_due > 0)
                    <a href="{{ route('booking.checkout', $booking) }}" class="btn-accent">Pay Remaining Balance</a>
                @endif
                <form method="POST" action="{{ route('account.bookings.cancel', $booking) }}"
                    ...
                    @csrf
                    <button type="submit" class="btn-outline !border-red-200 !text-red-600 hover:!border-red-400">Cancel Booking</button>
                </form>
            </div>
        @endif
    </div>
    ...
    <div class="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="card p-6">
            <div class="flex items-center justify-between">
                ...
                @if ($booking->invoice)
                    <a href="{{ route('account.invoices.show', $booking->invoice) }}" class="text-sm text-primary font-medium hover:underline">View Invoice</a>
                @endif
            </div>
