...
<div class="card p-6">
    ...
    <div class="mt-4 space-y-2 text-sm">
        <div class="flex justify-between"><span class="text-muted">Dewasa × {{ $booking->adult_count }}</span><span>Rp {{ number_format($booking->price_adult * $booking->adult_count, 0, ',', '.') }}</span></div>
        @if ($booking->child_count > 0)
            <div class="flex justify-between"><span class="text-muted">Anak-anak × {{ $booking->child_count }}</span><span>Rp {{ number_format($booking->price_child * $booking->child_count, 0, ',', '.') }}</span></div>
        @endif
        @if ($booking->infant_count > 0)
            <div class="flex justify-between"><span class="text-muted">Bayi × {{ $booking->infant_count }}</span><span>Rp {{ number_format($booking->price_infant * $booking->infant_count, 0, ',', '.') }}</span></div>
        @endif
        ...
        <div class="flex justify-between text-emerald-600"><span>Sudah Dibayar</span><span>Rp {{ number_format($booking->amount_paid, 0, ',', '.') }}</span></div>
